...
import { FinalPaymentModal } from '@/components/campaign/FinalPaymentModal';

export default function CampaignDetailsPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { user } = useAuthStore();
  const { campaigns, getCampaignsByAdvertiser, getCampaignsByOwner } = useCampaignStore();
  ...
  // Find campaign based on user role
  const userCampaigns = user?.role === 'advertiser' 
    ? getCampaignsByAdvertiser(user.id)
    : getCampaignsByOwner(user?.id || '');
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <Button onClick={() => navigate(user?.role === 'advertiser' ? '/advertiser/campaigns' : '/owner/campaigns')}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button
            variant="outline"
            size="sm"
            onClick={() => navigate(user?.role === 'advertiser' ? '/advertiser/campaigns' : '/owner/campaigns')}
          >
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back
          </Button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {user?.role === 'advertiser' ? 'Media Owner' : 'Advertiser'}
                </p>
              </div>
            </div>
          </div>
